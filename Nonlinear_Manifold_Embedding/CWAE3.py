...
def CWAE3(Y,X0,A,h,t,Noise,parameters):
    """
    CWAE variant 3 (CWAE3) filter.

    CWAE3 combines the chained encoding of CWAE2 with the latent-space conditioning
    of CWAE1: the x-encoder takes the latent z_y (not raw y), and the decoder
    conditions on z_y (not the decoded ambient ŷ):
        z_y  = ΦY(y),   z_xy = ΦX(z_y, x),   x̂ = GX(z_xy, z_y)
    At inference, encoder_y is applied to y_true to produce z_y_true, which is
    then passed directly to the decoder alongside a sampled z_xy ~ N(0, I):
        x̂ = GX(z_xy, ΦY(y_true))
    This differs from CWAE1 (where ΦX sees raw y) and CWAE2 (where GX conditions
    on the decoded ŷ = GY(z_y) and encoder_y is bypassed at inference).
    ...
    # Like CWAE2, this encoder chains off the encoded observation z_y rather than raw y.
    ...
    # Like CWAE1, this decoder conditions on z_y rather than raw y.
    class Decoder_XY(nn.Module):
        def __init__(self, input_dim, latent_dims, hidden_dim, num_resblocks=2):
            ...
            # Input layer: transforms concatenated (z_xy, z_y) to the hidden dimension.
    ...
    def train(encoder_y, encoder_xy, decoder_y, decoder_xy, discriminator, X_Train, Y_Train, iterations, learning_rate, ts, Ts, batch_size, k, K, latent_dims, lamb, n_critic):
        ...
        for i in range(iterations):
            ...
            # Decoder_XY conditions on z_y (latent), not raw y.
    ...
    for k in range(NUM_SIM):
        ...
        for i in range(T - 1):
            ...
            # Draw new particles: encode the true observation to z_y, then decode with random z_xy.
            with torch.no_grad():
