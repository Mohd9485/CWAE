...
def LREnKF(Y, X0, A, h, grad_h, t, Noise, r_dims, alpha=None, SIGMA=1e-6):
    ...
    # Flatten X0 because LREnKF does not care about spatial information
    X0 = np.reshape(X0, (NUM_SIM, math.prod(X0.shape[1:-1]), N))
    ...
    # Flatten Y because LREnKF does not care about spatial information
    Y = np.reshape(Y, (NUM_SIM, T, math.prod(Y.shape[2:-1]), 1))
